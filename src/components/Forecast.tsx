                ...
                (dayObject: DayWeatherData, dayIndex: number, daysArray) => {
                  ...
                  return (
                    <div
                      ...
                      className={` h-[20rem] max-w-max relative text-center duration-300 hover:scale-105 hover:my-6 lg:hover:scale-110 hover:z-20 flex flex-col justify-center bg-white/95 border-2 rounded-md  hover:border-slate-700/95  ${lgStylesCard}`}>
                      ...
                      <svg
                        ...
                        {/* ------------ */}
                        {dayObject.data.map(
                          (dayElement, timeIndex, timesArray) => {
                            ...
                            const lastPreviousDayTemp = () => {
                              if (dayIndex - 1 >= 0) {
                                const prevDayLastTemperature =
                                  daysArray[dayIndex - 1].data.slice(-1)[0]
                                    .weather.temp;
                            ...
                            const firstNextDayTemp = () => {
                              if (timeIndex + 1 < total) {
                                ...
                                return daysArray[dayIndex + 1].data[0].weather
                                  .temp;
                              } else {
                                return y1 - midY / 2;
                              }
                                ...
                                {isFirstTime && (
                                  <MainLine
                                    ...
                                    y={
                                      midY -
                                      (temp +
                                        tempDiff(lastPreviousDayTemp(), temp))
                                    }
                                    x2={divStep / 2}
                                    y2={y1}
                                  ...
                                  <>
                                    <MainLine
                                      ...
                                      y2={
                                        midY -
                                        (temp +
                                          tempDiff(firstNextDayTemp(), temp))
                                      }
                                    />
                                    <TemperatureLabel
                                      x={x2}
                                      ...
                                      temp={nextTemp()}
                                      description={
                                        dayElement.weather.description
                                      }
                                      icon={dayElement.weather.icon}
                                    />
                                    <TimeAxis
